...
class CurriculumGraphJsonExporter:
    """Exports an already loaded CurriculumGraph as a JSON document."""
    ...
    def export(self, graph: CurriculumGraph, output_dir: str | Path) -> dict[str, Path]:
        ...
        try:
            ...
            destination.write_text(
                self._serializer.serialize(graph, "json"),
                encoding="utf-8",
            )
            return {self.FILE_NAME: destination}
        except Exception as exc:
            raise CurriculumGraphJsonExporterError(
                f"Failed to export CurriculumGraph JSON file to: {output_path}"
            ) from exc
