...
class ChangelogSection(BaseModel):
    """Structured changelog section with entries."""
...
class ChangelogContent(BaseModel):
    ...
    sections: Dict[str, List[str]] = Field(
        ...,
        description="Changelog sections with entries",
    )
    summary: str = Field(
        ...,
                ...
                if len(parts) >= 2:
                    ...
                    if match:
                        commit = CommitInfo(
                            ...
                            scope=match.group("scope") or "core",
...
Provide the changelog content with appropriate sections."""
    ...
    def _format_commits_for_agent(
        ...
        for commit in commits:
            ...
            lines.append(f"- {commit.type}({commit.scope}){breaking}: {commit.subject}")
    ...
    def format_changelog_entry(
        ...
        lines = [f"## [{changelog.version}] - {changelog.date}\n"]

        # Order sections properly
        section_order = ["Breaking", "Added", "Changed", "Fixed", "Removed"]
        for section in section_order:
            if section in changelog.sections and changelog.sections[section]:
                lines.append(f"### {section}")
                for entry in changelog.sections[section]:
